...
def model_predict(tup):
    ...
    sin_lon = np.sin(np.pi / 180 * lon_new)
    ...
    #for monthIndex in range(0, 12):
        for depthIndex in range(0, 40):
            ...
            #_lon = np.tile(lon_new, 330).reshape(-1, 1).ravel()
            #_lon2 = np.tile(lon_new2, 330).reshape(-1, 1).ravel()
            _sin_lon = np.tile(sin_lon, 330).reshape(-1, 1).ravel()
            ...
            X_pred = np.c_[_sin_lon, _lat, _year, _month1, _depth, _u, _v, _w, _temp, _salt, _ssh, _taux, _tauy, _etop, _cmip6]
...
if __name__ == '__main__':
    ...
    model_list = ['et_400_20_fit_2db_f4_del1986_seed1_after1970_sin_lon_lt20_sw100.pkl']
