...
def model_predict(tup):
    ...
    cos_lon = np.cos(np.pi / 180 * lon_new)
    ...
    region_path = '../ocean_area_0.5deg.nc'
    sea_region_nc = Dataset(region_path)
    lat_360 = sea_region_nc.variables['lat'][:]
    lat_min = np.where(lat_360 == np.min(lat_new))[0][0]
    lat_max = np.where(lat_360 == np.max(lat_new))[0][0]
    sea_region = sea_region_nc.variables['area_num'][lat_min:lat_max+1]
    sea_region_ravel = sea_region.ravel()
    #Indian offshore
    #sea_region_list = (sea_region_ravel != 21)

    #Indian nearshore
    #sea_region_list = (sea_region_ravel != 15) & (sea_region_ravel != 17) & (sea_region_ravel != 19)
    
    #Indian offshore
    #sea_region_list = (sea_region_ravel != 14) & (sea_region_ravel != 16) & (sea_region_ravel != 18)
    ...
    #for monthIndex in [3-1, 6-1, 9-1]:
        for depthIndex in range(0, 40):
            month1 = monthIndex + 1
            _cos_lon = np.tile(cos_lon, 330).reshape(-1, 1).ravel()
            ...
            X_pred = np.c_[_cos_lon, _lat, _year, _month1, _depth, _u, _v, _w, _temp, _salt, _ssh, _taux, _tauy, _etop, _cmip6]
            ...
            mask_mul = np.matrix([True, True, True, True, True, True, True, True, True, True]).T
...
if __name__ == '__main__':
    ...
    model_list = ['et_400_None_2_fit_2dbf5_t_Indian_cos_lon.pkl', 'et_400_None_2_fit_2dbf5_t_seed1_after1970_Indian_cos_lon.pkl']
    ...
    for model_item in model_list:
        ...
        cdoshell = 'bash ./o2_sp_test_full.sh {0}'.format(model_name)
