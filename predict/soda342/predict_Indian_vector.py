...
def model_predict(tup):
    ...
    v[v < -100000000] = 0
    w[w < -100000000] = 0
    taux[taux < -100000000] = 0
    tauy[tauy < -100000000] = 0
    velocity = np.sqrt(np.power(u,2) + np.power(v,2) + np.power(w,2))
    tau = np.sqrt(np.power(taux,2) + np.power(tauy,2))
    ...
    #for monthIndex in [3-1, 6-1, 9-1]:
        for depthIndex in range(0, 50):
            ...
            _velocity = velocity[monthIndex, depthIndex, :].ravel()
            ...
            X_pred = np.c_[_lon, _lat, _year, _month, _depth, _temp, _salt, _ssh, _etop, _mlp, _mls, _mlt, _net_heating, _prho, _velocity, _tau]            
            #X_mask_data = np.c_[_u, _v, _w, _temp, _salt, _ssh, _taux, _tauy, _mlp, _mls, _mlt, _net_heating, _prho]
            X_mask_data = np.c_[_temp]
...
if __name__ == '__main__':
    ...
    model_list = ['et_200_None_2_fit_2db_Indian_train_index0_vector_feat.pkl']
                
    for model_item in model_list:
        model_pkl = model_item
        model_name = os.path.splitext(model_pkl)[0]
        model_path = r'../../model/' + model_pkl
        to_predict_path = r'/home/zju/do5/soda342/'
