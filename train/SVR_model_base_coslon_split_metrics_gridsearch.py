...
def train(seed, file_name, work_log):
    ...
    model_estimate = GridSearchCV(SVR(), param_grid={"kernel": ("linear", 'rbf'), "C": np.logspace(-3, 3, 7), "gamma": np.logspace(-3, 3, 7)})
...
if __name__ == '__main__':
    ...
    for db_item in db:
        ...
        test_data_path_after1970  = '../dataset/2db/{0}/GRIDDING_{0}_seed1_after1970_Arctic.csv'.format(db_item)
        test_data_path_before1970 = '../dataset/2db/{0}/GRIDDING_{0}_seed1_before1970_Arctic.csv'.format(db_item)
        test_data_path_other = '../dataset/2db/{0}/GRIDDING_{0}_seed1_other_Arctic.csv'.format(db_item)
        test_data_path_balance = '../dataset/2db/{0}/GRIDDING_{0}_seed1_balance_Arctic.csv'.format(db_item)
        ...
        train_data_name = '{0}_seed1_after1970_Arctic'.format(db_item)
        file_name = 'SVR_fit_{0}_cos_lon_gridsearch'.format(train_data_name)
