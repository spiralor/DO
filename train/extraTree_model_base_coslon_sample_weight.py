...
def train(seed, all_data_path, file_name, work_log):
    ...
    model_estimate=ExtraTreesRegressor(random_state=seed, n_jobs=-1,n_estimators=200,max_depth=20)
    
    sample_weight = np.full(len(train_Y),1)
    for index in range(len(train_Y)):
        if train_Y[index] <= 20:
            sample_weight[index] = 100

    model_estimate.fit(train_X, train_Y, sample_weight=sample_weight)
...
if __name__ == '__main__':
    all_data_path = '~/do4/do_train/dataset/2db/2dbf4_new/GRIDDING_2dbf4_new_seed1_after1970.csv'
    #all_data_path = '~/do4/do_train/dataset/2db/2db_f4_del1986_del_woa3/GRIDDING_2db_f4_del1986_ice_delwoa3_seed1_after1970.csv'
    #all_data_path = '~/do4/do_train/dataset/2db/2db_f4_ctdlinear/GRIDDING_2db_f4_ctdlinear_seed1_after1970.csv'
    file_name = 'et_200_20_fit_2dbf4_new_seed1_after1970_coslon_lt20_sw100'
