...
def read_split_data(all_data_path):
    ...
    for index in range(cv):
        ...
        tmp_data_x[:,0] = np.cos(np.pi/180 * tmp_data_x[:,0])
...
def train(seed, all_data_path, file_name):
    ...
    for n_estimators in [200]:   
        for max_depth in [None]:  
            for min_samples_split in [2,5,10]:
                print('n_estimators = ', n_estimators, ', max_depth = ', max_depth, ', min_sample_split = ', min_samples_split,', time = ',str(tm.strftime("%Y-%m-%d %H:%M:%S", tm.localtime())))
                predict_list =[]
                real_list = []
                for index in range(cv):
    ...
    # 指标验证
    cal_R2, cal_RMSE, cal_MAE, cal_MAPE, cal_r, count_r = cal_test_stats(test_X_after1970, test_Y_after1970, best_estimate)
    excel = excel.append( pd.Series({'count':count_r,'R2':cal_R2, 'RMSE':cal_RMSE , 'MAE':cal_MAE , 'MAPE':cal_MAPE , 'r':cal_r }, name= 'test_after1970'))
    cal_R2, cal_RMSE, cal_MAE, cal_MAPE, cal_r, count_r = cal_test_stats(test_X_before1970, test_Y_before1970, best_estimate)
    excel = excel.append( pd.Series({'count':count_r,'R2':cal_R2, 'RMSE':cal_RMSE , 'MAE':cal_MAE , 'MAPE':cal_MAPE , 'r':cal_r }, name= 'test_before1970')) 
    cal_R2, cal_RMSE, cal_MAE, cal_MAPE, cal_r, count_r = cal_test_stats(test_X_other, test_Y_other, best_estimate)
    excel = excel.append( pd.Series({'count':count_r,'R2':cal_R2, 'RMSE':cal_RMSE , 'MAE':cal_MAE , 'MAPE':cal_MAPE , 'r':cal_r }, name= 'test_other' )) 
...
if __name__ == '__main__':
    ...
    all_data_path = '../dataset/2db/2dbf5_t_bn/GRIDDING_2dbf5_t_bn_seed1_after1970.csv'
    test_data_path_after1970 = '../dataset/2db/2dbf5_t_bn/GRIDDING_2dbf5_t_bn_seed1_after1970.csv'
    test_data_path_before1970 = '../dataset/2db/2dbf5_t_bn/GRIDDING_2dbf5_t_bn_seed1_before1970.csv'
    test_data_path_other = '../dataset/2db/2dbf5_t_bn/GRIDDING_2dbf5_t_bn_seed1_other.csv'
    file_name = 'ets_fit_2dbf5_t_bn_seed1_after1970_in_best_params_200_none'
