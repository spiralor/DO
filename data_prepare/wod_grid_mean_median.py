...
#                             400, 400, 400, 400,
#                             500, 500])
# depthInsideList = np.array([50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 100,
#                              100, 100, 100, 100,
#                              100, 100, 100, 100,
#                              100, 100, 100, 100,
#                              100, 100, 100, 100,
#                              100, 100, 100, 100,
#                              100, 100])
depthInsideList = np.array([50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50,
                             50, 50, 50, 50,
                             100, 100, 100, 100,
                             100, 100, 100, 100,
                             100, 100, 100, 100,
                             100, 100, 100, 100,
                             100, 100])
# depthOutsideList = depthInsideList
# np.array(
...
# depthInsideList = np.array([50,50,50,50,50,50,50,50,50,50,50,50,50,50,
#                     50,100,100,100,100,100,100,200,200,200,200,
#                     200,1000,1000,1000,1000,1000,1000,1000,1000,
#                     1000,1000,1000,1000,1000,1000,])
#
# depthOutsideList = np.array([200,200,200,200,200,200,200,200,200,200,200,
#                     200,200,200,200,200,200,200,200,400,400,400,
#                     400,1000,1000,1000,1000,1000,1000,1000,1000,
#                     1000,1000,1000,1000,1000,1000,1000,1000,1000])

#depthInsideList = np.array([50,50,50,50,50,50,50,50,50,50,50,50,50,50,
#                            50,50,50,100,100,100,100,200,200,200,200,200,
...
def process(year):
    ...
    for month in range(1, 13):
        ...
        sql = (
            ...
            .format(str(year), str(month), db_f)
        cur.execute(sql)
        rows = cur.fetchall()
            ...
            for depthNumber in range(0, 40):
                ...
                if len(distanceMatrixIndex0[0]) != 0:
                    ...
                    insertSQL = "INSERT INTO \"GRIDDING_{7}_{8}_grid_intlev\" (\"Year\", \"Month\", \"Longitude\", \"Latitude\", \"depth(m)\", \"Oxy_mean\", \"Oxy_median\") " \
                        ...
                        .format(result[0], result[1], result[2], result[3], result[4], result[5], result[6], db + '_' + db_f, tag)
                ...
                if len(np.where(distanceMatrix == min(distanceMatrix[distanceMatrixIndex1]))[0]) != 0:
                    if len(np.where(distanceMatrix == min(distanceMatrix[distanceMatrixIndex1]))[0]) == 1:
                        ...
                        if (len(gridRow) - 1 < outsideBelow) or (not judgeDepthShred(gridRow[outsideBelow][4], depthList[depthNumber], 1)):
                        ...
                        # if len(np.where((distanceMatrix - gridRow[outsideBelow][4] - depthList[depthNumber])<0.0001)[0]) == 1:
                            ...
                            data = []
                            for pointIndex in \
                                    np.where(abs(distanceMatrix - gridRow[outsideBelow][4] + depthList[depthNumber]) < 0.000001)[
                                        0]:
                        ...
                        data = []
                        for pointIndex in \
                                np.where(abs(distanceMatrix - gridRow[insideBelow][4] + depthList[depthNumber]) < 0.000001)[0]:
                            data.append(gridRow[pointIndex][5])
                ...
                if len(np.where(distanceMatrix == max(distanceMatrix[distanceMatrixIndex2]))[0]) != 0:
                    if len(np.where(distanceMatrix == max(distanceMatrix[distanceMatrixIndex2]))[0]) == 1:
                        ...
                        if (0 > outsideAbove) or (not judgeDepthShred(gridRow[outsideAbove][4], depthList[depthNumber], 1)):
                            ...
                            data = []
                            for pointIndex in \
                                    np.where(abs(distanceMatrix - gridRow[outsideAbove][4] + depthList[depthNumber]) < 0.000001)[
                                        0]:
                                data.append(gridRow[pointIndex][5])
                        ...
                        data = []
                        for pointIndex in \
                                np.where(abs(distanceMatrix - gridRow[insideAbove][4] + depthList[depthNumber]) < 0.000001)[0]:
                            data.append(gridRow[pointIndex][5])
                ...
                if y_mean < min(pointSet_mean[1, 1], pointSet_mean[2, 1]) or y_mean > max(pointSet_mean[1, 1], pointSet_mean[2, 1]):
                    y_mean = flin(depthList[depthNumber], pointSet_mean[1:3, 0], pointSet_mean[1:3, 1])
                if y_median < min(pointSet_median[1, 1], pointSet_median[2, 1]) or y_median > max(pointSet_median[1, 1], pointSet_median[2, 1]):
                    y_median = flin(depthList[depthNumber], pointSet_median[1:3, 0], pointSet_median[1:3, 1])
