...
# Copyright (C) 2021 #
# @Time    : 2021/7/6 15:50
# @File    : WOD_GRID_mean_median_seperate_soda342.py
...
conn_database = "do"
conn_user = "postgres"
conn_password = "1q!@hyes0913"
conn_host = "222.205.21.45"
conn_port = "5432"
select_table = '2db_flag_v0_out_1980'
# insert_table = 'SODA342_2db_v0_i0_grid_intlev_copy1'
insert_table = 'SODA342_2db_v0_i0_grid_intlev'
...
def process(year):
    conn = psycopg2.connect(database=conn_database, user=conn_user, password=conn_password, host=conn_host,
                            port=conn_port)
    ...
    for month in range(1, 13):
        sql = (
            ...
            "from \"{2}\" where \"Year\"::int = {0} and \"Month\"::int = {1} order by"
            " \"Year\"::int,\"Month\"::int,\"depth(m)\"::float,\"Longitude\"::float,\"Latitude\"::float"). \
            format(year, month, select_table)
        cur.execute(sql)
        rows = cur.fetchall()
        ...
        for index in range(0, lon_dimension * lat_dimension):
            ...
            for depthNumber in range(depth_level):
                ...
                insertSQL = "INSERT INTO \"{7}\" (\"Year\", \"Month\", \"Longitude\", \"Latitude\", \"depth(m)\", \"Oxy_mean\", \"Oxy_median\") " \
                            "values('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}')" \
                    .format(result[0], result[1], result[2], result[3], result[4], result[5], result[6], insert_table)
                cur.execute(insertSQL)
                conn.commit()
